...
REWARD_MODEL_URL = os.getenv("REWARD_MODEL_URL", "http://[REDACTED_IP]:8400/score")
...
CAUTION_PATTERN = re.compile(r"证据有限|间接证据|不能得出强结论|不能得出强推荐|需谨慎|谨慎解释|尚不明确|缺乏直接比较")
STRONG_CLAIM_PATTERN = re.compile(r"明确优于|显著降低|显著改善|强烈推荐|一线首选|必须|证实|确定可以|应当优先")
...
def get_training_schedule(increment_rollout: bool) -> Dict[str, float]:
    ...
    format_scale = 0.7                        # 固定，不再动态衰减
    correct_scale = 1.0                       # 固定，v18 降低 format 权重，突出 citation/correctness

    if progress < 0.3:
        rm_weight = 0.0
    else:
        rm_weight = 0.25 * (progress - 0.3) / 0.7  # 0.0 -> 0.25
...
def check_evidence_safety(answer: str, valid_ids: Set[str]) -> float:
    """
    v18: 轻量 evidence-safety reward。
    - 没有真实引用支撑却使用强结论词：扣分
    - 没有 SR/RCT 级别引用时能谨慎表达证据有限：小加分
    """
    cited_ids = extract_answer_cites(answer)
    real_ids = cited_ids & valid_ids
    cited_prefixes = {rid[:2] for rid in real_ids}

    has_sr_or_rct = bool(cited_prefixes & {"03", "04"})
    has_guideline = bool(cited_prefixes & {"01", "02"})
    strong_claim = STRONG_CLAIM_PATTERN.search(answer) is not None
    cautious = (ABSTAIN_PATTERN.search(answer) is not None) or (CAUTION_PATTERN.search(answer) is not None)

    reward = 0.0

    if strong_claim and not real_ids:
        reward -= 0.35
    elif strong_claim and not has_sr_or_rct:
        reward -= 0.12

    if cautious and not has_sr_or_rct:
        reward += 0.15
    if cautious and has_guideline and not has_sr_or_rct:
        reward += 0.05

    reward = max(-0.5, min(0.25, reward))
    logger.info(
        f"[EvidenceSafety] strong={strong_claim}, cautious={cautious}, "
        f"prefixes={cited_prefixes}, score={reward:.3f}"
    )
    return reward
...
def compute_correctness_reward(answer: str, valid_ids: Set[str]) -> float:
    ...
    # v17_plus_small: small, non-saturating recall bonus
    # Keep total extra reward small to avoid v16-style 5.304 plateau.
    if valid_ids:
        recall = len(real_ids) / max(len(valid_ids), 1)
        recall_bonus = 0.10 * recall
        reward += recall_bonus
        logger.info(
            f"[RecallSmall] real={len(real_ids)}, total_valid={len(valid_ids)}, "
            f"recall={recall:.2f}, bonus={recall_bonus:.3f}"
        )

    # v17_plus_small: tiny citation diversity bonus, capped at 3 DB prefixes
    if real_ids:
        cited_db_prefixes = {rid[:2] for rid in real_ids}
        cited_db_count = len(cited_db_prefixes & {"01", "02", "03", "04"})
        diversity_bonus = 0.02 * min(cited_db_count, 3)
        reward += diversity_bonus
        logger.info(
            f"[CiteDiversitySmall] prefixes={cited_db_prefixes}, "
            f"db_count={cited_db_count}, bonus={diversity_bonus:.3f}"
        )

    if valid_ids and not real_ids and not ABSTAIN_PATTERN.search(answer):
        reward -= 0.20

    reward = max(-3.0, min(3.2, reward))
    return reward
