...
from sql_agent_v18 import SQLSearchAgent

DEFAULT_MODEL_PATH = os.getenv("DEFAULT_MODEL_PATH", "/workspace/models/Qwen2.5-14B-Instruct-SFT-Agent")
...
def verl_default_config(
    ...
    config: Dict[str, Any] = {
        ...
        "trainer": {
            ...
            "experiment_name": f"ebm_agent_14b_grpo_{n_gpus}gpu_v20_sft",
            "nnodes": 1,
            "save_freq": save_freq,
            "test_freq": test_freq,
    ...
    *,
    ...
    if trajectory_level:
        config["agentlightning"] = {
            "trace_aggregator": {
                "level": "trajectory",
                "trajectory_max_prompt_length": 12288,
...
def main() -> None:
    ...
    parser.add_argument(
        "--train-file",
        type=str,
        default="data/train_v18_full_weighted.parquet",
        help="训练集 parquet 路径",
    )
    parser.add_argument(
        "--val-file",
    ...
    parser.add_argument(
        "--progress-file",
        type=str,
        default="/tmp/sql_agent_progress_v20_sft.json",
